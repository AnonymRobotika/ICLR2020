...
class Callback:
    ...
    def on_test_end(self, loss: float , aux_loss: float, logs: Dict[str, Any] = None):
        pass
...
class ConsoleLogger(Callback):
    ...
    def on_test_end(self, loss: float, aux_loss: float, logs: Dict[str, Any] = None):
        if self.as_json:
            dump = dict(mode='test', epoch=self.epoch_counter, loss=self._get_metric(loss), aux_loss=self._get_metric(aux_loss))
            for k, v in logs.items():
                dump[k] = self._get_metric(v)
            output_message = json.dumps(dump)
        else:
            output_message = f'test: epoch {self.epoch_counter}, loss {loss}, aux_loss {aux_loss},  {logs}'
