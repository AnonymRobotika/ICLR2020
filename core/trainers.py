    ...
    def eval(self):
        ...
        with torch.no_grad():
            for batch in self.validation_data:
                ...
                optimized_loss, rest, auxiliary_loss = self.game(*batch)
                mean_loss += optimized_loss
                mean_aux_loss += auxiliary_loss
                mean_rest = _add_dicts(mean_rest, rest)
                n_batches += 1
        mean_loss /= n_batches
        mean_aux_loss /= n_batches
        mean_rest = _div_dict(mean_rest, n_batches)

        return mean_loss.item(), mean_rest, mean_aux_loss.item()
    ...
    def train(self, n_epochs):
        ...
        for epoch in range(self.start_epoch, n_epochs):
            ...
            if self.validation_data is not None and self.validation_freq > 0 and epoch % self.validation_freq == 0:
                ...
                validation_loss, rest, aux_loss = self.eval()
                for callback in self.callbacks:
                    callback.on_test_end(validation_loss, aux_loss, rest)
