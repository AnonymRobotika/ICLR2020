...
class RnnReceiverReinforce(nn.Module):
    ...
    def forward(self, message, input=None, lengths=None):
        ...
        return sample, logits, entropy, encoded
...
class SenderReceiverRnnReinforceAuxiliaryTask(nn.Module):

    def __init__(self, sender, receiver, guesser, loss, auxiliary_loss, sender_entropy_coeff, receiver_entropy_coeff,
                 length_cost=0.0):

        super(SenderReceiverRnnReinforceAuxiliaryTask, self).__init__()
        self.sender = sender
        self.receiver = receiver
    ...
    def forward(self, sender_input, labels, receiver_input=None):
        ...
        message_lengths = find_lengths(message)
        receiver_output, log_prob_r, entropy_r, last_hidden = self.receiver(message, receiver_input, message_lengths)

        guesser_output = self.guesser(prev_hidden)
        receiver_thoughts = last_hidden.clone().detach()

        loss, rest = self.loss(sender_input, message, receiver_input, receiver_output, labels)

        acc = rest["acc"]
        self.mean_acc = (self.mean_acc * 100 + acc)/(101)
        if self.mean_acc < 0.8:
             weight = 0.1
        #elif self.mean_acc < 0.4:
        #     weight = 0.1
        else:
             weight = 0.1
        auxiliary_loss, info = self.auxiliary_loss(receiver_thoughts, message, receiver_input, guesser_output, labels, weight)
        ...
        return optimized_loss, rest, auxiliary_loss.mean()
